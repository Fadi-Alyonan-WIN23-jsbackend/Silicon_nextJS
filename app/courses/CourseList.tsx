...
const CourseList: React.FC<CourseListProps> = ({ courses }) => {
  useEffect(() => {
    console.log('Rendering courses:', courses); // Logga de kurser som skickas till komponenten
  }, [courses]);
      ...
      <div className={`container ${styles.container}`}>
        ...
        <div className={`courseItems ${styles.courseItems}`}>
          {courses && courses.length > 0 ? (
            courses.map((item) => {
              console.log('Rendering course item:', item); // Logga varje kurs för detaljerad kontroll
              return (
